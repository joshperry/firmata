var assign = require("object-assign"),
    OneWireUtils = require('./onewireutils.js'),
...
var util = require("util"),
    ...
    protocolStream = require('./protocolStream');

function fromMidiBinary(buf, start, end) {
  buf = buf.slice(start, end);
  var len = buf.length;
  var bbuf = new Buffer(len/2);

  assert(len % 2 === 0);

  for(var i = 0; i < len; i += 2) {
    bbuf[i/2] = buf[i] | buf[i+1] << 7;
  }

  return bbuf;
}

function fromMidiString(buf, start, end) {
  return fromMidiBinary(buf, start, end).toString();
}

/**
 * RESPONSE contains functions to be called when we receive a MIDI message from the arduino.
 ...
 * @private
 * @param {Buffer} frame the MIDI from contents recieved from the firmata device
 * @param {Board} board the current arduino board we are working with.
 */

RESPONSE[consts.REPORT_VERSION] = function(frame, board) {
  board.version.major = frame[1];
  board.version.minor = frame[2];

  debug('REPORT_VERSION', board.version.major, board.version.minor);
...
RESPONSE[consts.ANALOG_MESSAGE] = function(frame, board) {
  var value = frame[1] | (frame[2] << 7);
  var pin = frame[0] & 0x0F;

  if (board.pins[board.analogPins[pin]]) {
    board.pins[board.analogPins[pin]].value = value;
  }

  debug('ANALOG_MESSAGE', pin, value);
...
RESPONSE[consts.DIGITAL_MESSAGE] = function(frame, board) {
  var port = (frame[0] & 0x0F);
  var portValue = frame[1] | (frame[2] << 7);
  ...
  for (var i = 0; i < 8; i++) {
    ...
    if (pin && (pin.mode === board.MODES.INPUT)) {
      ...
      debug('DIGITAL_MESSAGE', pinNumber, value);
...
/**
 * Handles SYSEX frames and dispatches them to the proper handler
 * @private
 * @param {Buffer} frame the MIDI from contents recieved from the firmata device
 * @param {Board} board the current arduino board we are working with
 */
RESPONSE[consts.START_SYSEX] = function(frame, board) {
  var cmd = frame[1];
  var response = RESPONSE[cmd];

  debug('SYSEX');

  if(response)
    response(frame.slice(2), board);
};
...
RESPONSE[consts.QUERY_FIRMWARE] = function(frame, board) {
  board.firmware.version = {
    major: frame[0],
    minor: frame[1]
  };
  board.firmware.name = fromMidiString(frame, 2);

  debug('QUERY_FIRMWARE', board.firmware);
...
RESPONSE[consts.CAPABILITY_RESPONSE] = function(frame, board) {
  ...
  if (!board.pins.length) {
    for (var i = 0, n = 0; i < frame.length; i++) {
      if (frame[i] === 127) {
        var modesArray = [];
      ...
      if (n === 0) {
        supportedModes |= (1 << frame[i]);
      }
      n ^= 1;
    }
  }

  debug('CAPABILITY_RESPONSE');
...
RESPONSE[consts.PIN_STATE_RESPONSE] = function(frame, board) {
  var pin = frame[0];
  board.pins[pin].mode = frame[1];
  board.pins[pin].state = frame[2];
  if (frame.length > 4) {
    board.pins[pin].state |= (frame[3] << 7);
  }
  if (frame.length > 5) {
    board.pins[pin].state |= (frame[4] << 14);
  }

  debug('PIN_STATE_RESPONSE');
...
RESPONSE[consts.ANALOG_MAPPING_RESPONSE] = function(frame, board) {
  var pin = 0;
  var currentValue;
  for (var i = 0; i < frame.length; i++) {
    currentValue = frame[i];
...
RESPONSE[consts.I2C_REPLY] = function(frame, board) {
  var address = fromMidiBinary(frame, 0, 1)[0];
  var register = fromMidiBinary(frame, 2, 3)[0];
  var reply = fromMidiBinary(frame, 4);
...
RESPONSE[consts.ONEWIRE_DATA] = function(frame, board) {
  var subCommand = frame[0];

  if (!RESPONSE[consts.subCommand]) {
    return;
  }

  debug('ONEWIRE_DATA');

  RESPONSE[consts.subCommand](frame.slice(1), board);
};

RESPONSE[consts.ONEWIRE_SEARCH_REPLY] = function(frame, board) {
  var pin = frame[0];
  var replyBuffer = frame.slice(1, frame.length);

  debug('ONEWIRE_SEARCH_REPLY');

  board.emit("1-wire-search-reply-" + pin, OneWireUtils.readDevices(replyBuffer));
};

RESPONSE[consts.ONEWIRE_SEARCH_ALARMS_REPLY] = function(frame, board) {
  var pin = frame[0];
  var replyBuffer = frame.slice(1, frame.length);

  debug('ONEWIRE_SEARCH_ALARMS_REPLY');

  board.emit("1-wire-search-alarms-reply-" + pin, OneWireUtils.readDevices(replyBuffer));
};

RESPONSE[consts.ONEWIRE_READ_REPLY] = function(frame, board) {
  var encoded = frame.slice(1, frame.length);
  var decoded = Encoder7Bit.from7BitArray(encoded);
  var correlationId = (decoded[1] << 8) | decoded[0];

  debug('ONEWIRE_READ_REPLY');
...
RESPONSE[consts.STRING_DATA] = function(frame, board) {
  var string = fromMidiString(frame, 0);

  debug('STRING_DATA', string);
...
RESPONSE[consts.PULSE_IN] = function(frame, board) {
  var pin = fromMidiBinary(0, 1)[0];
  var duration = fromMidiBinary(2, 9).readInt32BE();
...
RESPONSE[consts.STEPPER] = function(frame, board) {
  var deviceNum = frame[0];
 ...
 * @param {StreamSource} streamSource This is a function that will provide a stream to communicate with the arduino.
 ...
 */
var Board = function(streamSource, options, callback) {
  Emitter.call(this);
  ...
  // Make the streamSource backwards compatible
  if(typeof streamSource !== 'function') {
    if(typeof streamSource === 'string') {
      // String must be the com port name
      options = options.serialport || {
          baudRate: 57600,
          bufferSize: 1
        };
      options.comname = streamSource;

      streamSource = require('./serialStream')(options);
    } else if(typeof streamSource === 'object') {
      // Object must be a precreated serialport
      var port = streamSource;
  ...
  this.sp
    .pipe(protocolStream(this))
    .on('frame', function(frame) {
      var cmd = frame[0];
      // For frames that are not version, the upper nibble is the command
      cmd = cmd === consts.REPORT_VERSION ? cmd : cmd & 0xF0;

      debug('midi command', cmd, frame);
      var response = RESPONSE[cmd];
      if(response)
        response(frame, board);
      else
        debug('no response parser found');
    });
...
Board.prototype.reportVersion = function(callback) {
  ...
  this.sp.write(new Buffer([consts.REPORT_VERSION]));
};
...
Board.prototype.queryFirmware = function(callback) {
  ...
  this.sp.write(new Buffer([consts.START_SYSEX, consts.QUERY_FIRMWARE, consts.END_SYSEX]));
};
...
Board.prototype.analogWrite = function(pin, value) {
  ...
  if (pin > 15) {
    data[0] = consts.START_SYSEX;
    data[1] = consts.EXTENDED_ANALOG;
    data[2] = pin;
    data[3] = value & 0x7F;
    data[4] = (value >> 7) & 0x7F;
    ...
    data[data.length] = consts.END_SYSEX;
  } else {
    data.push(consts.ANALOG_MESSAGE | pin, value & 0x7F, (value >> 7) & 0x7F);
  }
...
Board.prototype.pinMode = function(pin, mode) {
  ...
  this.sp.write(new Buffer([consts.PIN_MODE, pin, mode]));
};
...
Board.prototype.digitalWrite = function(pin, value) {
  ...
  this.sp.write(new Buffer([consts.DIGITAL_MESSAGE | port, portValue & 0x7F, (portValue >> 7) & 0x7F]));
};
...
Board.prototype.queryCapabilities = function(callback) {
  ...
  this.sp.write(new Buffer([consts.START_SYSEX, consts.CAPABILITY_QUERY, consts.END_SYSEX]));
};
...
Board.prototype.queryAnalogMapping = function(callback) {
  ...
  this.sp.write(new Buffer([consts.START_SYSEX, consts.ANALOG_MAPPING_QUERY, consts.END_SYSEX]));
};
...
Board.prototype.queryPinState = function(pin, callback) {
  ...
  this.sp.write(new Buffer([consts.START_SYSEX, consts.PIN_STATE_QUERY, pin, consts.END_SYSEX]));
};
...
Board.prototype.sendString = function(string) {
  var data = [];
  data.push(consts.START_SYSEX);
  data.push(consts.STRING_DATA);
  for (var i = 0, length = string.length; i < length; i++) {
    data.push(string[i] & 0x7F);
    data.push((string[i] >> 7) & 0x7F);
  }
  data.push('\0');
  data.push(consts.END_SYSEX);
  this.sp.write(data);
};
...
Board.prototype.sendI2CConfig = function(delay) {
  delay = delay || 0;
  this.sp.write(new Buffer([consts.START_SYSEX, consts.I2C_CONFIG, (delay & 0xFF), ((delay >> 8) & 0xFF), consts.END_SYSEX]));
};
...
Board.prototype.sendOneWireConfig = function(pin, enableParasiticPower) {
  this.sp.write(new Buffer([consts.START_SYSEX, consts.ONEWIRE_DATA, consts.ONEWIRE_CONFIG_REQUEST, pin, enableParasiticPower ? 0x01 : 0x00, consts.END_SYSEX]));
};
...
Board.prototype.sendOneWireSearch = function(pin, callback) {
  this._sendOneWireSearch(consts.ONEWIRE_SEARCH_REQUEST, "1-wire-search-reply-" + pin, pin, callback);
};
...
Board.prototype.sendOneWireAlarmsSearch = function(pin, callback) {
  this._sendOneWireSearch(consts.ONEWIRE_SEARCH_ALARMS_REQUEST, "1-wire-search-alarms-reply-" + pin, pin, callback);
};

Board.prototype._sendOneWireSearch = function(type, event, pin, callback) {
  this.sp.write(new Buffer([consts.START_SYSEX, consts.ONEWIRE_DATA, type, pin, consts.END_SYSEX]));
...
Board.prototype.sendOneWireRead = function(pin, device, numBytesToRead, callback) {
  ...
  this._sendOneWireRequest(pin, consts.ONEWIRE_READ_REQUEST_BIT, device, numBytesToRead, correlationId, null, null, "1-wire-read-reply-" + correlationId, function(data) {
...
Board.prototype.sendOneWireReset = function(pin) {
  this._sendOneWireRequest(pin, consts.ONEWIRE_RESET_REQUEST_BIT);
};
...
Board.prototype.sendOneWireWrite = function(pin, device, data) {
  this._sendOneWireRequest(pin, consts.ONEWIRE_WRITE_REQUEST_BIT, device, null, null, null, Array.isArray(data) ? data : [data]);
};
...
Board.prototype.sendOneWireDelay = function(pin, delay) {
  this._sendOneWireRequest(pin, consts.ONEWIRE_DELAY_REQUEST_BIT, null, null, null, delay);
};
...
Board.prototype.sendOneWireWriteAndRead = function(pin, device, data, numBytesToRead, callback) {
  ...
  this._sendOneWireRequest(pin, consts.ONEWIRE_WRITE_REQUEST_BIT | consts.ONEWIRE_READ_REQUEST_BIT, device, numBytesToRead, correlationId, null, Array.isArray(data) ? data : [data], "1-wire-read-reply-" + correlationId, function(data) {
    clearTimeout(readTimeout);
...
Board.prototype._sendOneWireRequest = function(pin, subcommand, device, numBytesToRead, correlationId, delay, dataToWrite, event, callback) {
  ...
  if (device || numBytesToRead || correlationId || delay || dataToWrite) {
    subcommand = subcommand | consts.ONEWIRE_WITHDATA_REQUEST_BITS;
  }
  ...
  var output = [consts.START_SYSEX, consts.ONEWIRE_DATA, subcommand, pin];
  output = output.concat(Encoder7Bit.to7BitArray(bytes));
  output.push(consts.END_SYSEX);
...
Board.prototype.setSamplingInterval = function(interval) {
  ...
  this.sp.write(new Buffer([consts.START_SYSEX, consts.SAMPLING_INTERVAL, (safeint & 0xFF), ((safeint >> 8) & 0xFF), consts.END_SYSEX]));
};
...
Board.prototype.reportAnalogPin = function(pin, value) {
  if (value === 0 || value === 1) {
    ...
    this.sp.write(new Buffer([consts.REPORT_ANALOG | pin, value]));
  }
};
...
Board.prototype.reportDigitalPin = function(pin, value) {
  ...
  if (value === 0 || value === 1) {
    ...
    this.sp.write(new Buffer([consts.REPORT_DIGITAL | port, value]));
  }
};
...
Board.prototype.reset = function() {
  this.sp.write(new Buffer([consts.SYSTEM_RESET]));
};

module.exports = {
  Board: Board,
  serialStreamSource: require('./serialStream'),
  RESPONSE: RESPONSE,
};
