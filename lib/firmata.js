 ...
 * @private
 * @param {Buffer} frame the SYSEX from from the hardware
 * @param {Board} board the current arduino board we are working with.
 */
...
var Board = function(streamSource, options, callback) {
  ...
  this.sp
    ...
    .on('frame', function(frame) {
      var cmd = frame[0];
      if(cmd === consts.START_SYSEX) {
        debug('got sysex frame');
        cmd = frame[1];
        frame = frame.slice(2);
      } else {
        // For non-sysex and version frames, the upper nibble is the command
